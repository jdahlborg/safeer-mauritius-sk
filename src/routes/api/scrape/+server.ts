...
export const GET: RequestHandler = async ({ url }) => {
	...
	const transaction_type = url.searchParams.get('transaction_type') ?? 'buy';
	...
	const { listings, error } = await source.collect({ transaction_type, propertyType: property_type, sortBy: sort_by, pages });
