<script lang="ts">
	...
	async function updateStatus(id: number, status: string) {
		...
		savedListings = savedListings.map(l => l.id === id ? { ...l, status } : l);
...
<svelte:head>
	<title>Listings — Safeer Admin</title>
</svelte:head>
...
<div class="pb-16">
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="mb-8 flex items-center justify-between">
			<div>
				<h1 class="text-3xl font-bold text-gray-900" style="font-family:'Playfair Display',serif">Listings</h1>
						...
						<div class="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col">
							<div class="relative h-44 bg-gray-100">
								...
								{#if listing.status === 'sold'}
									<span class="absolute bottom-2 left-2 text-xs font-bold px-2 py-0.5 rounded-full bg-gray-800 text-white">SOLD</span>
								{:else if listing.status === 'rented'}
									<span class="absolute bottom-2 left-2 text-xs font-bold px-2 py-0.5 rounded-full bg-purple-600 text-white">RENTED</span>
								{/if}
							</div>
							<div class="p-4 flex flex-col flex-1">
								<h3 class="font-semibold text-gray-900 text-sm leading-snug mb-1 line-clamp-2">{listing.title}</h3>
								...
								<select
									class="form-input text-xs py-1.5 mt-1 mb-2"
									value={listing.status ?? 'active'}
									onchange={(e) => updateStatus(listing.id, (e.target as HTMLSelectElement).value)}
								>
